"""This module processes PDF pages and extracts layout information."""
...
def process_pdf_pages(pdf_images, bboxes_list, classes_list, scores_list, id_to_names):
    ...
    for page_idx, (bboxes, classes, scores) in enumerate(
        zip(bboxes_list, classes_list, scores_list)
    ):
        page_elements = []
        ...
        page_data = {"page_number": page_idx + 1, "elements": page_elements}
        pages_data.append(page_data)

    # Create final JSON structure
    output = {"document_layout": {"total_pages": len(pages_data), "pages": pages_data}}
