...
def crop_image(page_image, bbox):
    ...
    try:
        ...
        padding = min(width, height) // 200  # Adaptive padding (0.5% of smaller dimension)
        ...
        if crop_box[2] <= crop_box[0] or crop_box[3] <= crop_box[1] or \
           crop_box[2] - crop_box[0] < min_dimension // 2 or crop_box[3] - crop_box[1] < min_dimension // 2:
        ...
        if crop_width < min_dimension or crop_height < min_dimension:
            ...
            print(f"DEBUG: Resizing small crop to {new_width}x{new_height} for better OCR")
    ...
    for page_idx, page_image in enumerate(page_images):
        ...
        sorted_elements = sorted(elements, 
                                key=lambda x: (x.get('confidence', 0), 
                                              (x['bbox']['x2'] - x['bbox']['x1']) * (x['bbox']['y2'] - x['bbox']['y1'])), 
                                reverse=True)
        ...
        for elem in sorted_elements:
            ...
            for prev_area in processed_areas:
                ...
                if x1 < x2 and y1 < y2:
                    ...
                    curr_area = (curr_bbox['x2'] - curr_bbox['x1']) * (curr_bbox['y2'] - curr_bbox['y1'])
            ...
            if not significant_overlap:
                ...
                processed_areas.append((curr_bbox['x1'], curr_bbox['y1'], 
                                      curr_bbox['x2'], curr_bbox['y2']))
        ...
        for element_idx, element in enumerate(elements):
            ...
            if not all(isinstance(v, (int, float)) for v in [bbox['x1'], bbox['y1'], bbox['x2'], bbox['y2']]):
            ...
            if element_type in ['figure_caption', 'caption']:
                ...
                if caption_height > height * 0.2:  # Caption shouldn't be more than 20% of page height
            ...
            if element_type == 'table':
                ...
                if page_idx == 0 and bbox['y1'] < height * 0.25:  # Main title at top of first page, increased threshold
                    is_valid_title = True
                elif bbox['y1'] < height * 0.2:  # Section titles at top of other pages, increased threshold
                    is_valid_title = True
                elif prev_element_type != 'title' and len(elements) > element_idx + 1:  # Check spacing
                    next_element = elements[element_idx + 1]
                    spacing = next_element['bbox']['y1'] - bbox['y2']
                    # Consider both spacing and relative position
                    if spacing > height * 0.015 and bbox['y2'] - bbox['y1'] < height * 0.1:
                        is_valid_title = True
                ...
                if prev_element_type == 'figure':
                    ...
                    print("DEBUG: Caption without figure, using general text extraction")
                    extracted_text = extract_text_with_easyocr(cropped_image)
            else:
            ...
            if extracted_text:
                print(f"DEBUG: Successfully extracted text for element {element_idx + 1}")
                print(f"DEBUG: Element type: {element_type}, Text preview: {extracted_text[:50]}...")
            else:
